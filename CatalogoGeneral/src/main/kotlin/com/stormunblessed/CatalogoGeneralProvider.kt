...
/**
 * Catálogo General: meta provider para descubrir todo el contenido de TMDB en
 * español de España (es-ES). Al cargar, busca el título en TODAS las extensiones
 * instaladas (excepto ella misma y otros meta providers) y, si lo encuentra,
 * delega la reproducción en ellas. Las extensiones del mismo idioma tienen prioridad.
 */
class CatalogoGeneralProvider : TmdbProvider() {
    ...
    // Idioma de los datos de TMDB (catálogo, títulos, sinopsis)
    private val tmdbLanguage = "es-ES"

    // Idioma de respaldo cuando TMDB no tiene traducción (p. ej. sinopsis vacía)
    private val tmdbFallbackLanguage = "es-MX"

    private fun filterName(name: String): String {
        val noAccents = Normalizer.normalize(name, Normalizer.Form.NFD)
            .replace(Regex("""\p{Mn}+"""), "")
        return Regex("""[^a-zA-Z0-9-]""").replace(noAccents, "")
    }

    private fun comparableName(name: String): String {
        val stripped = filterName(Regex("""\s*\(?\d{4}\)?\s*$""").replace(name, ""))
        // Títulos que son solo un año (1917, 2012...) no deben quedar vacíos
        return stripped.ifEmpty { filterName(name) }
    }

    // Nombres de las extensiones a consultar, por orden de prioridad.
// Vacía = consulta todas.
...
private val validApis
    get() = apis
        .filter { it::class != this::class && it.providerType != ProviderType.MetaProvider }
        .let { list ->
            if (allowedProviders.isEmpty()) {
                list.sortedByDescending { it.lang == this.lang }
            } else {
                list
                    .filter { api -> allowedProviders.any { it.equals(api.name, ignoreCase = true) } }
                    .sortedBy { api -> allowedProviders.indexOfFirst { it.equals(api.name, ignoreCase = true) } }
            }
        }

    private fun apiNames(list: List<com.lagradost.cloudstream3.MainAPI>): String =
    list.map { "'${it.name}'" }.joinToString(", ")
    private val providerSearchTimeoutMs = 5_000L
    private val findMatchesDeadlineMs = 8_000L
    private val maxMatchesForFallback = 6
    ...
    private suspend fun searchProvider(
        ...
        return try {
            ...
            // Throwable: algunas extensiones lanzan NotImplementedError (es un Error, no una Exception)
            logError(e)
            null
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val path = if (data.isTv) "/tv/$id" else "/movie/$id"
        val details = parseJson<TmdbGeneralDetails>(tmdbGet(path))
        // Mismos datos en es-MX: respaldo para sinopsis/imágenes sin traducción a es-ES
        val detailsFallback = parseJson<TmdbGeneralDetails>(
            tmdbGet(path, mapOf("language" to tmdbFallbackLanguage))
        )

        val title = details.displayTitle.takeIf { it.isNotBlank() }
            ?: detailsFallback.displayTitle.takeIf { it.isNotBlank() }
            ?: return null
        val type = if (data.isTv) TvType.TvSeries else TvType.Movie
        val year = data.year ?: details.year
        val poster = getImageUrl(details.posterPath ?: detailsFallback.posterPath)
        val backdrop = getImageUrl(details.backdropPath ?: detailsFallback.backdropPath, "w780")
        val score = Score.from10(details.voteAverage)
        val tags = (details.genres ?: detailsFallback.genres)?.mapNotNull { it.name }
        val overview = details.overview?.takeIf { it.isNotBlank() }
            ?: detailsFallback.overview?.takeIf { it.isNotBlank() }
        val runtime = details.runtime ?: detailsFallback.runtime

        // Títulos a probar en las demás extensiones: es-ES, es-MX y original
        val searchYear = year
        val candidateTitles = listOf(
            title,
            detailsFallback.displayTitle,
            details.originalTitle ?: details.originalName,
        )
            .filterNotNull()
            .filter { comparableName(it).isNotEmpty() }
            .distinctBy { comparableName(it).lowercase() }

        var matches = emptyList<SearchMatch>()
        for (candidate in candidateTitles) {
            matches = findMatches(candidate, comparableName(candidate), searchYear, data.isTv)
        ...
        if (matches.isEmpty()) {
            return if (data.isTv) {
                newTvSeriesLoadResponse(title, url, type, episodes = emptyList()) {
                    ...
                    this.year = year
                    this.plot = "Instaladas: " + apiNames(apis) + "\nConsultadas: " + apiNames(validApis)
                    this.score = score
                    this.tags = tags
                }
            } else {
                newMovieLoadResponse(title, url, type, dataUrl = "") {
                    ...
                    this.score = score
                    this.tags = tags
                    this.duration = runtime?.let { it * 60 }
                }
            }
        }
    ...
    private suspend fun tmdbGet(path: String, params: Map<String, String> = emptyMap()): String {
        ...
        val body = app.get(
            ...
            params = buildMap {
                ...
                putAll(params) // si params trae "language", sustituye al idioma por defecto
            },
        ).text
        tmdbCache[key] = TmdbCacheEntry(now, body)
        if (tmdbCache.size > 300) {
            tmdbCache.clear()
        }
        return body
    }
